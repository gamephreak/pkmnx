...
export class Teams extends pkmn.Teams {
  static validateTeam(team: pkmn.Team, format?: Format): string[] {
    ...
    if (rules.clauses.has('Baton Pass') && bp > 1) {
      problems.push(`Team has ${3} Pokémon with Baton Pass despite Baton Pass Clause's limit of 1.`);
    }

    // Drizzle ++ Swift Swim, Drought ++ Chlorophyll
    if (rules.format.id == 'gen5ou') {
      if (abilityTable['drizzle'] && abilityTable['swiftswim']) {
        problems.push('Drizzle and Swift Swim may not be used to the same team.');
      }
      if (abilityTable['drought'] && abilityTable['chlorophyll']) {
        problems.push('Drought and Chlorophyll may not be used to the same team.');
      }
    }
